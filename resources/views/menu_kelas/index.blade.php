...
<script>
    ...
    async function getISS() {
        ...
        id = data.reduce(function(result, current) {
            result[current.nama] = result[current.nama] || [];
            result[current.nama].push(current);
            return result;
        }, {});
        ...
        $.each(id, function(index, row) {
            ...
            html += `<a href="{{route('kelas')}}?id=` + row[0].uuid + `"  class="text-dark"><h5><strong><u>` + row[0].nama + '</strong></u>' + '(' + row[0].jml_kelas + ')' + ' </h5></a><p>' + row[0].deskripsi + '</p>';
            html += '<div class="position-relative form-group"><a data-toggle="modal" data-target=".bd-example-modal-sm" class="btn-icon  btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav" href="#" onclick="getEdit(\'' + row[0].uuid + '\')">Edit</a></div>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
